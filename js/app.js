...
/* ★ 参考框排序：文件(上传序)在前，消息(加入序)在后 —— 保证缓存前缀稳定 */
...
function renderRefPool(){const bar=document.getElementById('refBar');const list=document.getElementById('refList');if(!bar||!list)return;const pool=sortedRefPool();if(!pool.length){bar.style.display='none';list.innerHTML='';return;}bar.style.display='flex';list.innerHTML='';pool.forEach((r)=>{const item=document.createElement('span');item.className='ref-item'+(r.checked?' ref-checked':'');const cb=document.createElement('input');cb.type='checkbox';cb.checked=!!r.checked;cb.className='ref-cb';cb.onchange=()=>{r.checked=cb.checked;const c=curChat();if(c)c.updatedAt=Date.now();scheduleSave();renderRefPool();};item.appendChild(cb);const icon=r.kind==='file'?(r.type==='document'?'📄':r.type==='table'?'📊':r.type==='image'?'🖼️':'📝'):'💬';let nm=r.name||'';if(r.kind==='file'&&nm.length>8)nm=nm.slice(0,8)+'…';const label=document.createElement('span');label.className='ref-name';label.textContent=icon+nm;label.title=r.name;item.appendChild(label);        if(r.kind==='message'){const rn=document.createElement('button');rn.className='ref-btn';rn.textContent='✏️';rn.title='改备注';rn.onclick=()=>renameRefMsg(r);item.appendChild(rn);}const rm=document.createElement('button');rm.className='ref-btn ref-rm';rm.textContent='×';rm.onclick=()=>{const c=curChat();const idx=c.refPool.indexOf(r);if(idx>-1)c.refPool.splice(idx,1);c.updatedAt=Date.now();scheduleSave();renderRefPool();};item.appendChild(rm);list.appendChild(item);});}
/* 消息参考项改名：保留"问题N/回答N"前缀，追加自定义备注 */
function renameRefMsg(r){
    // 提取前缀（问题N / 回答N），后面的·备注可改
...
/* 消息区 */
function renderMs(){const area=document.getElementById('msgsArea');const c=curChat();if(!c){area.innerHTML='<div class="empty"><div class="ico">🚀</div><p>请先新建一个对话</p></div>';return;}UI.renderMessages(area,c.messages,{onDelete:(m)=>{c.messages=c.messages.filter(x=>x!==m);c.updatedAt=Date.now();scheduleSave();renderMs();},onRegen:(m)=>regenerate(m),onContinue:(m)=>continueGenerate(m),onEdit:(m)=>editUserMsg(m),selectMode:_refSelectMode,selectedMsgs:_selectedMsgs,onSelectToggle:(mid,checked)=>{if(checked){if(!_selectedMsgs.includes(mid))_selectedMsgs.push(mid);}else _selectedMsgs=_selectedMsgs.filter(x=>x!==mid);}});appendUsageInfo(area,c);const ti=document.getElementById('titleIn');if(ti)ti.value=c.title||'';const pb=document.querySelector('#pinBtn .hi');if(pb)pb.textContent=c.isPinned?'📍':'📌';renderChatTotal(c);}
...
function addEng(){const name=prompt('新私有引擎名称：','我的引擎');if(!name||!name.trim())return;const id='custom_'+gId().slice(0,8);S.profiles[id]={id,name:name.trim(),origin:'private',engineType:'chat',protocol:'openai',authType:'bearer',base:'https://api.openai-proxy.org/v1',key:'',model:'gpt-4o-mini',useTemp:false,temperature:0.7,useMax:false,max_tokens:4096,useTopP:false,top_p:1,useFreq:false,frequency_penalty:0,useCache:false,cacheTTL:'5m',priceIn:0,priceOut:0,priceCacheRead:0,priceCacheWrite:0};S.currentEngId=id;scheduleSave();renderEngTabs();renderEngForm();renderSB();renderMs();}
function delEng(){const p=S.profiles[S.currentEngId];if(!p||p.origin!=='private'){toast('公有引擎不可删除','er');return;}if(!confirm('删除私有引擎 '+p.name+'？'))return;delete S.profiles[p.id];const all=allEngines();S.currentEngId=all.length?all[0].id:'';scheduleSave();renderEngTabs();renderEngForm();renderSB();renderMs();toast('已删除');}
...
async function continueGenerate(msg) {
    ...
    const aiMsgs = (c.messages || []).filter(m => m.role === 'assistant');
    ...
    _streamCtrl = API.streamChat(profile, sendMsgs, {
        ...
        onDone: async (full, usage) => {
            ...
            msg._interrupted = false;
            if (usage) {
                const prev = msg._usage || { inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0 };
                msg._usage = {
...
/* 编辑用户问题：改内容 → 删除这条及之后所有 → 用新内容重发（批次3b） */
async function editUserMsg(msg) {
    ...
    const nv = prompt('编辑你的问题（确定后将删除此消息之后的所有回复并重新生成）：', oldText);
    if (nv === null) return;
...
function hKey(e){
    if(e.key==='Escape'){hideCmdPanel();return;}
    if(e.key==='Enter'&&!e.shiftKey&&!e.isComposing){
        const panel=document.getElementById('cmdPanel');
        if(panel&&panel.classList.contains('show')){e.preventDefault();return;}
...
function onInputChange(el){
    ...
    const val=el.value;
    if(val.startsWith('/')){
        showCmdPanel(val.slice(1).toLowerCase());
    }else{
        hideCmdPanel();
    }
}
function showCmdPanel(kw){
    ...
    panel.innerHTML=list.map(c=>{
        ...
        return '<div class="cmd-item" onclick="applyCmd('+idx+')"><div class="cmd-name">'+esc(c.name)+'</div><div class="cmd-preview">'+esc(c.content.slice(0,50))+'</div></div>';
    }).join('');
...
async function loadGlobalConfig(){
    try{
        ...
        if(data.ok&&data.config){
            ...
            if(data.config.quickCmds){
                _quickCmds=String(data.config.quickCmds).split('\n').map(line=>{
                    const p=line.split('|');
...
document.addEventListener('click',(e)=>{const panel=document.getElementById('cmdPanel');const inp=document.getElementById('uIn');if(panel&&panel.classList.contains('show')&&e.target!==inp&&!panel.contains(e.target))hideCmdPanel();});
