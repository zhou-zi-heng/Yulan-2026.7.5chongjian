...
let S = {
    ...
    defaultMode: 'free', folders: [], folderCollapsed: {}, _engineMigrated: false,
};
...
let _attChunk=false;
let _refSelectMode=false;
let _selectedMsgs=[];
var _wfAlertCtx=null;
let _dragChatId=null;
let _publicEngines=[];   // ★ 公有引擎（后端下发，不含Key）
let _userPerm={};        // ★ 当前用户权限
...
async function loadState(){
    ...
    if(loaded&&typeof loaded==='object'){
        S=Object.assign({profiles:{},chats:{},chatOrder:[],currentChatId:null,currentEngId:'claude',theme:'light',snapInterval:5,userName:'',archiveInterval:10,uiMode:'chat',defaultMode:'free',folders:[],folderCollapsed:{},_engineMigrated:false},loaded);
    ...
    // ★ 首次登录清空本地旧引擎（只清引擎，不碰对话）
    if(!S._engineMigrated){
        S.profiles={};   // 清空本地私有引擎（旧的公司引擎会由公有引擎替代）
        S._engineMigrated=true;
        console.log('[Migrate] 已清空本地旧引擎配置，改用公有引擎');
    }
...
function fixProfileFields(){
    for(const id in S.profiles){const p=S.profiles[id];
        if(p.useTemp===undefined)p.useTemp=false;if(p.useMax===undefined)p.useMax=false;
        ...
        p.authType='bearer';p.origin='private';
        if(p.useCache===undefined)p.useCache=false;
        p.cacheTTL=(p.protocol==='anthropic')?'1h':'5m';
        if(p.priceIn===undefined)p.priceIn=0;if(p.priceOut===undefined)p.priceOut=0;
        if(p.priceCacheRead===undefined)p.priceCacheRead=0;if(p.priceCacheWrite===undefined)p.priceCacheWrite=0;
    }
}

/* ===== 引擎：公有+私有统一访问 ===== */
/* 公有引擎从 _publicEngines（后端下发）；私有从 S.profiles */
function allEngines(){
    const list=[];
    _publicEngines.forEach(e=>list.push(e));
    Object.values(S.profiles).forEach(p=>list.push(p));
    return list;
}
function getEngineById(id){
    let e=_publicEngines.find(x=>x.id===id);
    if(e)return e;
    return S.profiles[id]||null;
}
function curProfile(){
    // 当前引擎：优先按 currentEngId 找
    let e=getEngineById(S.currentEngId);
    if(e)return e;
    // 回退第一个可用引擎
    const all=allEngines();
    return all.length?all[0]:null;
}
/* 公有引擎：用户本地覆盖的model（存 localStorage） */
function getPubEngineModel(engId){
    try{const m=JSON.parse(localStorage.getItem('feifan_pubmodel')||'{}');return m[engId]||'';}catch(e){return '';}
}
function setPubEngineModel(engId,model){
    try{const m=JSON.parse(localStorage.getItem('feifan_pubmodel')||'{}');m[engId]=model;localStorage.setItem('feifan_pubmodel',JSON.stringify(m));}catch(e){}
}
/* 公有引擎：用户本地覆盖的运行参数（存 localStorage） */
function getPubEngineParams(engId){
    try{const m=JSON.parse(localStorage.getItem('feifan_pubparams')||'{}');return m[engId]||{};}catch(e){return {};}
}
function setPubEngineParams(engId,params){
    try{const m=JSON.parse(localStorage.getItem('feifan_pubparams')||'{}');m[engId]=params;localStorage.setItem('feifan_pubparams',JSON.stringify(m));}catch(e){}
}
    ...
    try{
        ...
        if(data.ok&&Array.isArray(data.engines)){
            _publicEngines=data.engines.map(e=>{
                // 应用用户本地覆盖的model和参数
                const userModel=getPubEngineModel(e.id);
                const userParams=getPubEngineParams(e.id);
                return Object.assign({
                    engineType:'chat',useCache:false,cacheTTL:(e.protocol==='anthropic')?'1h':'5m',
                    useTemp:false,temperature:0.7,useMax:false,max_tokens:4096,useTopP:false,top_p:1,useFreq:false,frequency_penalty:0,
                },e,userParams,userModel?{model:userModel}:{});
            });
        }
    }catch(e){console.warn('[loadPublicEngines]',e);_publicEngines=[];}
    // 确保 currentEngId 有效
    if(!getEngineById(S.currentEngId)){
        const all=allEngines();
        if(all.length)S.currentEngId=all[0].id;
    }
}
...
function guardSensitive(inputText, sceneInfo){
    ...
    if(hit){
        const c=curChat();
        fireAlert('⚠️ 敏感词拦截\n用户：'+(S.userName||(Auth&&Auth.getUser?Auth.getUser().name:'')||'未署名')
            +'\n对话：《'+((c&&c.title)||'未命名')+'》\n场景：'+(sceneInfo||'工作流')
...
function setMode(mode){
    ...
    if(!confirm('确定将本对话切换为「'+modeLabel(target)+'」模式？\n\n发送第一条消息后将永久锁定。')){S.uiMode=(cur==='workflow')?'workflow':'chat';renderMode();return;}
...
function allowedGroups(){
    const all=(typeof Workflow!=='undefined'&&Workflow.isLoaded())?Workflow.getGroups():[];
    const ag=_userPerm.allowGroups;
    if(!ag||!ag.length)return all;  // 空=全部
    return all.filter(g=>ag.includes(g));
}
function renderWorkflow(){
    if(typeof Workflow==='undefined'||!Workflow.isLoaded()){
        const box=document.getElementById('wfCmds');if(box)box.innerHTML='<div style="color:var(--text2);font-size:12px;padding:8px">⚠️ 预设库未加载</div>';return;
    }
    ...
    if(grpSel){
        ...
        if(prev&&(prev==='__all__'||groups.includes(prev)))grpSel.value=prev;
        grpSel.dataset.filled='1';
    }
    const kw=(document.getElementById('wfSearch')?document.getElementById('wfSearch').value:'')||'';
    let presets=Workflow.getPresets(_wfGroup,kw);
    // 权限过滤：只显示允许分组的预设
    const ag=_userPerm.allowGroups;
    if(ag&&ag.length)presets=presets.filter(p=>ag.includes(p.group));
...
function renderWfAtts(){
    const box=document.getElementById('wfAttList');const cnt=document.getElementById('wfAttCount');
    if(!box)return;
    const c=curChat();
    ...
    let statusHtml='<div style="font-size:11px;padding:4px 6px;margin-bottom:4px;border-radius:5px;background:var(--pri-l);color:var(--text2);line-height:1.6">'
        +'🔄 持续参考：<b style="color:'+(contOn?'#10b981':'#999')+'">'+(contOn?'开（进知识库）':'关（进参考框）')+'</b>';
    if(kbCount>0)statusHtml+='<br>📚 知识库常驻：<b style="color:var(--pri)">'+kbCount+' 个文件</b>';
    statusHtml+='</div>';
    if(!_pendingAtts.length){box.innerHTML=statusHtml+'<div style="font-size:11px;color:var(--text2);padding:2px 0">（文件已进参考框）</div>';if(cnt)cnt.textContent='';return;}
    ...
    _pendingAtts.forEach((a,idx)=>{
        ...
        if(_attChunk&&a.type!=='image'&&a.text){const pv=document.createElement('button');pv.className='ai-rm';pv.textContent='👁';pv.title='预览打标';pv.style.color='var(--pri)';pv.style.flexShrink='0';pv.onclick=()=>previewChunkObj(a);item.appendChild(pv);}
        const rm=document.createElement('button');rm.className='ai-rm';rm.textContent='×';rm.title='移除';rm.style.flexShrink='0';rm.onclick=()=>{_pendingAtts.splice(idx,1);renderWfAtts();renderAttList();};
...
async function wfSend(stepId){
    ...
    scope.querySelectorAll('textarea[data-kind="input"][id^="wfin_'+stepId+'_"]').forEach(el=>{const seg=parseInt(el.getAttribute('data-seg'),10);inputsMap[seg]=el.value;if(el.value&&el.value.trim())joinedInput+=' '+el.value.trim();});
    let blankMissing=false;
    scope.querySelectorAll('.wf-blank').forEach(bl=>{const seg=parseInt(bl.getAttribute('data-seg'),10);const vals=[];bl.querySelectorAll('.wf-blank-in').forEach(inEl=>{const v=(inEl.textContent||'').trim();vals.push(v);if(!v)blankMissing=true;if(v)joinedInput+=' '+v;});inputsMap[seg]=vals;});
    ...
    let built;
    try{built=await Workflow.buildSend(_wfPresetId,stepId,inputsMap);}catch(e){toast('指令解密失败：'+e.message,'er');return;}
    ...
    await coreSend({visibleText:built.displayText,actualText:built.sendText,titleHint:built.stepName,_wfLeakCheck:true,atts:attsForWf,forceEngId:forceEngId});
    const inputs=Workflow.getInputs(_wfPresetId,stepId);
    inputs.forEach(inp=>{if(inp.kind==='input'){const el=document.getElementById('wfin_'+stepId+'_'+inp.segIndex);if(el)el.value=inp.defaultValue||'';}});
    scope.querySelectorAll('.wf-blank-in').forEach(inEl=>{inEl.textContent='';});
}

/* ===== 文件夹管理 ===== */
function addFolder(){const name=prompt('新建文件夹名称：','新文件夹');if(!name||!name.trim())return;const id='fd_'+gId().slice(0,8);S.folders.push({id:id,name:name.trim()});scheduleSave();renderSB();}
...
function delFolder(fid){const f=S.folders.find(x=>x.id===fid);if(!f)return;if(!confirm('删除文件夹「'+f.name+'」？\n\n文件夹内的对话会移回未分类。'))return;for(const cid in S.chats){if(S.chats[cid].folderId===fid)S.chats[cid].folderId=null;}S.folders=S.folders.filter(x=>x.id!==fid);delete S.folderCollapsed[fid];scheduleSave();renderSB();}
...
function _makeChatLi(id){
    ...
    li.draggable=true;li.dataset.cid=id;li.onclick=()=>switchChat(id);
...
function renderSB(){
    ...
    function match(c){if(!search)return true;const hay=(c.title+' '+(c.messages||[]).map(m=>typeof m.content==='string'?m.content:'').join(' ')).toLowerCase();return hay.includes(search);}
    if(search){
        if(folderArea)folderArea.style.display='none';document.getElementById('pinLbl').style.display='none';document.getElementById('arcLbl').style.display='none';
        let n=0;order.forEach(id=>{const c=S.chats[id];if(!match(c))return;chatList.appendChild(_makeChatLi(id));n++;});
        if(!n)chatList.innerHTML='<li style="font-size:12px;color:var(--text2);padding:8px">无匹配对话</li>';renderBadge();return;
    }
    ...
    if(folderArea){
        S.folders.forEach(f=>{
            ...
            const fEl=document.createElement('div');fEl.className='folder'+(collapsed?' collapsed':'');fEl.dataset.fid=f.id;
            ...
            const fr=document.createElement('button');fr.textContent='✏️';fr.onclick=(e)=>{e.stopPropagation();renameFolder(f.id);};
            const fdd=document.createElement('button');fdd.textContent='🗑️';fdd.onclick=(e)=>{e.stopPropagation();delFolder(f.id);};
            fActs.appendChild(fr);fActs.appendChild(fdd);hdr.appendChild(fActs);fEl.appendChild(hdr);
            ...
            fEl.appendChild(ul);folderArea.appendChild(fEl);
        });
    }
    ...
    ['今天','昨天','本周','更早'].forEach(g=>{
        if(!groups[g].length)return;const key='dg_'+g;const collapsed=!!S.folderCollapsed[key];
        ...
        chatList.appendChild(lbl);if(!collapsed)groups[g].forEach(id=>chatList.appendChild(_makeChatLi(id)));
    });
...
function renderBadge(){
    ...
    document.getElementById('badge').innerHTML='当前引擎: <strong>'+esc(p.name)+'</strong> '+originTag+esc(protoTag)+'<br>模型: '+esc(p.model||'-');
}
...
function renderRefPool(){
    const bar=document.getElementById('refBar');const list=document.getElementById('refList');if(!bar||!list)return;
    const pool=curRefPool();
    if(!pool.length){bar.style.display='none';list.innerHTML='';return;}
    bar.style.display='flex';list.innerHTML='';
    pool.forEach((r,idx)=>{
        const item=document.createElement('span');item.className='ref-item'+(r.checked?' ref-checked':'');
        const cb=document.createElement('input');cb.type='checkbox';cb.checked=!!r.checked;cb.className='ref-cb';
        ...
        const label=document.createElement('span');label.className='ref-name';label.textContent=icon+nm;label.title=r.name;item.appendChild(label);
        if(r.kind==='file'&&r.type!=='image'&&r.text){const pv=document.createElement('button');pv.className='ref-btn';pv.textContent='👁';pv.title='预览打标';pv.onclick=()=>previewChunkObj({fileName:r.name,type:r.type,text:r.text});item.appendChild(pv);}
        const rm=document.createElement('button');rm.className='ref-btn ref-rm';rm.textContent='×';rm.title='移除';rm.onclick=()=>{pool.splice(idx,1);const c=curChat();if(c)c.updatedAt=Date.now();scheduleSave();renderRefPool();};item.appendChild(rm);
        list.appendChild(item);
    });
}
function addFileToRefPool(parsed){const c=curChat();if(!c)return;if(!Array.isArray(c.refPool))c.refPool=[];c.refPool.push({id:gId(),kind:'file',name:parsed.fileName,type:parsed.type,text:parsed.text||'',dataUrl:parsed.dataUrl||null,meta:parsed.meta||{},checked:true,addedAt:Date.now()});c.updatedAt=Date.now();scheduleSave();renderRefPool();}
...
function addSelectedToRefPool(){
    ...
    if(!Array.isArray(c.refPool))c.refPool=[];let n=0;
    _selectedMsgs.forEach(mid=>{const m=c.messages.find(x=>x.id===mid);if(!m)return;const txt=typeof m.content==='string'?m.content:'';if(!txt)return;const roleLabel=m.role==='user'?'问题':'回答';const sameRole=c.messages.filter(x=>x.role===m.role);const roundNo=sameRole.indexOf(m)+1;c.refPool.push({id:gId(),kind:'message',name:roleLabel+roundNo,type:'text',text:txt,checked:true,addedAt:Date.now()});n++;});
    c.updatedAt=Date.now();scheduleSave();
    _refSelectMode=false;_selectedMsgs=[];const btn=document.getElementById('refSelBtn');if(btn){btn.style.color='';btn.style.background='';}const bar=document.getElementById('selActionBar');if(bar)bar.classList.remove('show');
    renderMs();renderRefPool();toast('✅ 已加入 '+n+' 项到参考框');
}
...
function renderMs(){
    ...
    UI.renderMessages(area,c.messages,{
        ...
        selectMode:_refSelectMode,selectedMsgs:_selectedMsgs,
...
function renderChatTotal(chat){
    ...
    if(!el){const acts=document.querySelector('.hdr-acts');if(!acts)return;el=document.createElement('span');el.id='chatTotalTag';el.style.cssText='font-size:11px;color:var(--text2);margin-right:8px;white-space:nowrap;align-self:center';acts.parentNode.insertBefore(el,acts);}
    ...
    el.textContent='💰 '+txt;
    el.title='入'+t.inputTokens+' 出'+t.outputTokens+(t.cacheReadTokens?' 缓存命中'+t.cacheReadTokens:'')+(t.cacheWriteTokens?' 缓存写入'+t.cacheWriteTokens:'');
}
function appendUsageInfo(area,chat){
    const nodes=area.querySelectorAll('.msg.assistant');let idx=0;
    const aiMsgs=(chat.messages||[]).filter(m=>m.role==='assistant');
    nodes.forEach(node=>{const m=aiMsgs[idx++];if(!m||!m._usage)return;const mm=node.querySelector('.mm');if(!mm||mm.querySelector('.usage-tag'))return;const tag=document.createElement('span');tag.className='usage-tag';tag.style.cssText='font-size:10px;color:var(--text2);opacity:.85';tag.innerHTML=formatUsage(m._usage,m._engId);mm.appendChild(tag);});
}
function formatUsage(u,engId){
    if(!u)return '';
    const input=u.inputTokens||0,output=u.outputTokens||0,total=input+output;const cacheRead=u.cacheReadTokens||0,cacheWrite=u.cacheWriteTokens||0;
    const parts=['⬆入'+input,'⬇出'+output,'Σ总'+total];if(cacheRead)parts.push('💰命中'+cacheRead);if(cacheWrite)parts.push('✍写'+cacheWrite);
    let costStr='';const cost=calcMsgCost(u,engId);if(cost!=null&&cost>0)costStr=' ≈$'+cost.toFixed(4)+' (¥'+(cost*USD_TO_CNY).toFixed(4)+')';
    return ' | '+parts.join(' ')+costStr;
}
function calcMsgCost(u,engId){
    if(!u)return null;if(u.platformCost!=null)return u.platformCost;
    const p=engId?getEngineById(engId):null;if(!p||!(p.priceIn||p.priceOut||p.priceCacheRead||p.priceCacheWrite))return null;
    const input=u.inputTokens||0,output=u.outputTokens||0,cacheRead=u.cacheReadTokens||0,cacheWrite=u.cacheWriteTokens||0;
    let inputCost;if((u.mode||p.protocol||'openai')==='openai')inputCost=Math.max(0,input-cacheRead-cacheWrite)/1e6*(p.priceIn||0);else inputCost=input/1e6*(p.priceIn||0);
...
function calcChatTotal(chat){
    ...
    chat.messages.forEach(m=>{if(m.role!=='assistant'||!m._usage)return;const u=m._usage;r.hasUsage=true;r.inputTokens+=u.inputTokens||0;r.outputTokens+=u.outputTokens||0;r.cacheReadTokens+=u.cacheReadTokens||0;r.cacheWriteTokens+=u.cacheWriteTokens||0;const c=calcMsgCost(u,m._engId);if(c!=null){r.cost+=c;r.hasCost=true;}});
    r.totalTokens=r.inputTokens+r.outputTokens+r.cacheReadTokens+r.cacheWriteTokens;return r;
}
...
/* ===== 引擎配置界面（公有只读关键项 + 私有可编辑） ===== */
function renderEngTabs(){
    const tabs=document.getElementById('engTabs');if(!tabs)return;tabs.innerHTML='';
    allEngines().forEach(p=>{
        const b=document.createElement('button');b.className='tab'+(p.id===S.currentEngId?' act':'');
        b.textContent=(p.origin==='public'?'📦':'👤')+p.name;
...
function renderEngForm(){
    ...
    const p=curProfile();
    if(!p){form.innerHTML='<p style="color:var(--text2)">⚠️ 暂无可用引擎，请联系管理员在超管后台为你配置引擎</p>';return;}
    const isPublic=p.origin==='public';
    if(isPublic)renderPublicEngForm(form,p);
    else renderPrivateEngForm(form,p);
}

/* 公有引擎：Base/Key隐藏只读，模型可改可获取，4参数可调 */
function renderPublicEngForm(form,p){
    form.innerHTML=`
        <div style="padding:10px;background:var(--pri-l);border-radius:8px;margin-bottom:12px;font-size:12px;color:var(--text2)">📦 这是公司配置的公有引擎，密钥由公司管理（你看不到）。你可以更换模型、调整下方参数。</div>
        ...
        <div class="fg"><label>🔑 API Key</label><input type="text" value="•••••• 公司已配置（隐藏）" disabled style="opacity:.7"></div>
        <div class="fg"><label>🧠 模型 ID <span style="font-size:11px;color:var(--pri)">（可改可获取）</span></label><div style="display:flex;gap:6px"><input type="text" id="engModel" value="${esc(p.model||'')}" placeholder="模型ID" style="flex:1"><button class="btn btn-s" onclick="fMdlsPublic('${esc(p.id)}')" id="fMdlsBtn" style="white-space:nowrap">🔄 获取</button></div><div id="mdlSel" style="display:none;margin-top:6px"></div></div>
        ${renderRuntimeParams(p)}
...
function renderPrivateEngForm(form,p){
    form.innerHTML=`
        <div style="padding:10px;background:var(--pri-l);border-radius:8px;margin-bottom:12px;font-size:12px;color:var(--text2)">👤 这是你自己添加的私有引擎（密钥存本地，随快照迁移）。</div>
        ...
        ${renderRuntimeParams(p)}
        <div style="margin-top:16px;padding-top:14px;border-top:1px solid var(--border)"><h4 style="font-size:13px;margin-bottom:8px">💵 费用单价（美元/1M token）</h4>
            <div class="fr"><div class="fg"><label>输入</label><input type="number" id="engPriceIn" value="${p.priceIn||0}" step="0.01"></div><div class="fg"><label>输出</label><input type="number" id="engPriceOut" value="${p.priceOut||0}" step="0.01"></div></div>
            <div class="fr"><div class="fg"><label>缓存读</label><input type="number" id="engPriceCR" value="${p.priceCacheRead||0}" step="0.01"></div><div class="fg"><label>缓存写</label><input type="number" id="engPriceCW" value="${p.priceCacheWrite||0}" step="0.01"></div></div>
        </div>
...
/* 运行参数（公有私有共用，4参数默认不勾选，可调） */
function renderRuntimeParams(p){
    return `<div style="margin-top:16px;padding-top:14px;border-top:1px solid var(--border)"><h4 style="font-size:13px;margin-bottom:10px">⚙️ 运行参数（默认不启用，可自行勾选调整）</h4>
...
function bindEngEvents(p){
    ...
    bind('engUseCache','onchange',e=>{const b=document.getElementById('engCacheBox');if(b)b.style.display=e.target.checked?'':'none';});
...
/* 保存公有引擎的用户设置（model+参数存本地） */
function savePublicEng(engId){
    const model=document.getElementById('engModel').value.trim();
    if(model)setPubEngineModel(engId,model);
    const params={
        useTemp:document.getElementById('engUseTemp').checked,temperature:parseFloat(document.getElementById('engTemp').value),
        useMax:document.getElementById('engUseMax').checked,max_tokens:parseInt(document.getElementById('engMax').value,10),
        useTopP:document.getElementById('engUseTopP').checked,top_p:parseFloat(document.getElementById('engTopP').value),
        useFreq:document.getElementById('engUseFreq').checked,frequency_penalty:parseFloat(document.getElementById('engFreq').value),
    };
    setPubEngineParams(engId,params);
    // 更新内存
    const e=_publicEngines.find(x=>x.id===engId);if(e){e.model=model||e.model;Object.assign(e,params);}
    renderEngTabs();renderSB();toast('✅ 已保存你的设置');
}
/* 私有引擎保存 */
function saveEng(){
    ...
    const ceb=document.getElementById('engUseCache');if(ceb)p.useCache=ceb.checked;
    p.cacheTTL=(p.protocol==='anthropic')?'1h':'5m';
    p.priceIn=parseFloat(document.getElementById('engPriceIn').value)||0;p.priceOut=parseFloat(document.getElementById('engPriceOut').value)||0;
    p.priceCacheRead=parseFloat(document.getElementById('engPriceCR').value)||0;p.priceCacheWrite=parseFloat(document.getElementById('engPriceCW').value)||0;
    scheduleSave();renderEngTabs();renderSB();renderMs();toast('✅ 已保存');
}
/* 公有引擎获取模型（走后端） */
async function fMdlsPublic(engId){
    const btn=document.getElementById('fMdlsBtn');btn.disabled=true;btn.textContent='⏳';
    try{
        const token=Auth.getToken();
        const resp=await fetch('/api/engines/models?engineId='+encodeURIComponent(engId),{headers:{'X-Auth-Token':token}});
        const data=await resp.json();
        if(!data.ok)throw new Error(data.error||'获取失败');
        const list=data.models||[];if(!list.length){toast('未返回模型','er');return;}
        const sel=document.getElementById('mdlSel');sel.style.display='';
        sel.innerHTML='<select id="mdlPick" style="width:100%;padding:7px;border:1px solid var(--border);border-radius:6px;background:var(--bg);color:var(--text)">'+list.map(m=>'<option value="'+esc(m)+'">'+esc(m)+'</option>').join('')+'</select>';
...
async function fMdls(){
    const p=S.profiles[S.currentEngId];if(!p)return;
    p.protocol=document.getElementById('engProto').value;p.base=document.getElementById('engBase').value.trim();p.key=document.getElementById('engKey').value.trim();
    if(!p.base||!p.key){toast('请先填 Base URL 和 API Key','er');return;}
    const btn=document.getElementById('fMdlsBtn');btn.disabled=true;btn.textContent='⏳';
    try{const list=await API.fetchModels(p);if(!list.length){toast('未返回模型','er');return;}const sel=document.getElementById('mdlSel');sel.style.display='';sel.innerHTML='<select id="mdlPick" style="width:100%;padding:7px;border:1px solid var(--border);border-radius:6px;background:var(--bg);color:var(--text)">'+list.map(m=>'<option value="'+esc(m)+'"'+(m===p.model?' selected':'')+'>'+esc(m)+'</option>').join('')+'</select>';document.getElementById('mdlPick').onchange=(e)=>{document.getElementById('engModel').value=e.target.value;};toast('✅ 已获取 '+list.length+' 个模型');}catch(e){toast('获取失败：'+e.message,'er');}finally{btn.disabled=false;btn.textContent='🔄 获取';}
}
function addEng(){
    const name=prompt('新私有引擎名称：','我的引擎');if(!name||!name.trim())return;
...
function delEng(){
    const p=S.profiles[S.currentEngId];if(!p||p.origin!=='private'){toast('公有引擎不可删除','er');return;}
    if(!confirm('删除私有引擎 '+p.name+'？'))return;
    delete S.profiles[p.id];const all=allEngines();S.currentEngId=all.length?all[0].id:'';scheduleSave();renderEngTabs();renderEngForm();renderSB();renderMs();toast('已删除');
}

/* ===== 全局设置 ===== */
function renderGlobalSettings(){
    const uIn=document.getElementById('userNameIn');if(uIn){const au=Auth&&Auth.getUser?Auth.getUser():null;uIn.value=S.userName||(au?au.name:'')||'';}
...
function renderArchiveInfo(){
    ...
    if(Archive.isEnabled()){const authTxt=Archive.isAuthorized()?'🟢 已授权':'🔴 待授权';el.innerHTML='✅ 已开启自动存档（'+authTxt+'）<br>目录：<strong>'+esc(Archive.getDirName())+'</strong>';}
...
async function renderStorageInfo(){
    ...
    try{const info=await DB.getStorageInfo();const au=Auth&&Auth.getUser?Auth.getUser():null;el.innerHTML='当前登录：<strong>'+(au?esc(au.name||au.username):'-')+'</strong>'+(Auth.isAdmin?(Auth.isAdmin()?'（👑管理员）':''):'')+'<br>已用：<strong>'+info.usedText+'</strong> / '+info.quotaText+'（'+info.percent+'%）<br>版本：'+APP_VERSION+'<br><button class="btn btn-s btn-d" onclick="Auth.logout()" style="margin-top:6px">🚪 退出登录</button>';}
    catch(e){el.textContent='存储信息获取失败';}
}

/* ===== 物理打标 ===== */
function toggleChunk(){_attChunk=!_attChunk;const btn=document.getElementById('chunkBtn');if(btn){btn.style.color=_attChunk?'#667eea':'';btn.style.background=_attChunk?'var(--pri-l)':'';btn.title='物理打标（'+(_attChunk?'开':'关')+'）';}toast(_attChunk?'📐 物理打标已开启':'📐 物理打标已关闭');renderRefPool();renderAttList();}
...
function previewSend(){
    ...
    let kbText='';if(c.knowledgeBase&&c.knowledgeBase.length){c.knowledgeBase.forEach(k=>{if(k.text)kbText+='\n[知识库]'+k.name+'（'+cntW(k.text)+'字）';});}
    if(kbText)sys+='\n【知识库资料】'+kbText;
    out+='━━━ SYSTEM ━━━\n'+(sys||'（空）')+'\n\n';
    let ref='';curRefPool().forEach(r=>{if(!r.checked)return;if(r.kind==='file'&&r.type==='image'){ref+='\n[图片] '+r.name;return;}if(!r.text)return;let body=r.text;if(r.kind==='file'&&_attChunk&&typeof Chunker!=='undefined')body=Chunker.chunk(r.text,{}).marked;ref+='\n=== '+(r.kind==='file'?'📎'+r.name:'💬'+r.name)+' ===\n'+body+'\n';});
    out+='━━━ 参考框（勾选项）━━━\n'+(ref||'（无勾选项）')+'\n\n';
    const inpVal=(document.getElementById('uIn')?document.getElementById('uIn').value:'')||'（输入框当前为空）';
    out+='━━━ 本轮问题 ━━━\n'+inpVal+'\n\n━━━ 说明 ━━━\n实际发送还含历史对话。工作流隐藏指令不展示。';
    const ta=document.getElementById('chunkPreviewTA');if(ta)ta.value=out;
    const h=document.querySelector('#mo-chunk-preview .md-h h2');if(h)h.textContent='📤 发送内容预览';openM('chunk-preview');
}
...
async function coreSend(opts){
    ...
    if(c.knowledgeBase&&c.knowledgeBase.length){c.knowledgeBase.forEach(k=>{if(k.type==='image')kbImages.push({fileName:k.name,dataUrl:k.dataUrl,type:'image'});else if(k.text){let body=k.text;if(_attChunk&&typeof Chunker!=='undefined')body=Chunker.chunk(k.text,{}).marked;kbText+='\n\n=== 📚 知识库：'+k.name+' ===\n'+body+'\n=== 知识库结束 ===\n';}});}

    let refText='';const refImages=[];
    curRefPool().forEach(r=>{if(!r.checked)return;if(r.kind==='file'&&r.type==='image'&&r.dataUrl){refImages.push(r);return;}if(!r.text)return;let body=r.text;if(r.kind==='file'&&_attChunk&&typeof Chunker!=='undefined')body=Chunker.chunk(r.text,{}).marked;const tag=r.kind==='file'?'📎参考文件：'+r.name:'💬参考片段·'+r.name;refText+='\n\n=== '+tag+' ===\n'+body+'\n=== 结束 ===\n';});
    ...
    const aiMsg={id:gId(),role:'assistant',content:'',_streaming:true,_time:nowTime(),_engId:engId};
    ...
    if((bareTitle===''||bareTitle==='新对话')&&c.messages.length<=2)c.title=buildTitleWithSuffix((opts.titleHint||visibleText||'新对话').slice(0,24),chatMode(c));
    ...
    _streamCtrl=API.streamChat(profile,sendMsgs,{
        ...
        onDone:async(full,usage)=>{
            ...
            if(opts._wfLeakCheck&&typeof Workflow!=='undefined'&&Workflow.isLeak(full)){
                ...
                const ctx=_wfAlertCtx||{};const cc=curChat();const reply20=String(full).replace(/\s+/g,'').slice(0,20);
                fireAlert('🚨 输出乱码警报\n用户：'+(ctx.user||'未署名')+'\n对话：《'+((cc&&cc.title)||'未命名')+'》\n预设：'+(ctx.preset||'-')+'\n步骤：'+(ctx.step||'-')+'\n他此前输入：'+(ctx.input||'-')+'\nAI回复前20字：'+reply20+'\n相似度：'+Workflow.similarityToLast(full)+'%\n时间：'+new Date().toLocaleString());
            }
            aiMsg.content=finalText;aiMsg._streaming=false;if(usage)aiMsg._usage=usage;
            c.updatedAt=Date.now();_streamCtrl=null;sendBtn.classList.remove('stop');sendBtn.textContent='➤';
            UI.fullRender(lastMsgEl,finalText);await saveNow();renderMs();renderSB();
            reportLog(c,profile,usage);   // ★ 监视上报
...
/* ★ 监视上报（精简统计） */
function reportLog(chat,profile,usage){
    try{
        const rounds=Math.floor((chat.messages||[]).filter(m=>m.role==='assistant').length);
        const tokens=usage?((usage.inputTokens||0)+(usage.outputTokens||0)):0;
        const token=Auth&&Auth.getToken?Auth.getToken():'';
        fetch('/api/log',{method:'POST',headers:{'Content-Type':'application/json','X-Auth-Token':token},body:JSON.stringify({chatName:chat.title||'',rounds:rounds,tokens:tokens,model:(profile&&profile.model)||''})}).catch(()=>{});
    }catch(e){}
}
...
async function send(){
    ...
    if(profile&&profile.engineType==='image'){if(!text){toast('请输入图片描述','er');return;}inp.value='';aRsz(inp);await coreSendImage(text);return;}
...
async function coreSendImage(prompt){
    let c=curChat();if(!c){newChat();c=curChat();}
    const profile=curProfile();if(!profile){toast('无可用引擎','er');return;}
    const userMsg={id:gId(),role:'user',content:prompt,_actual:prompt,_time:nowTime()};c.messages.push(userMsg);
    const aiMsg={id:gId(),role:'assistant',content:'🎨 正在生成图片...',_streaming:true,_time:nowTime(),_engId:S.currentEngId,_isImage:true};c.messages.push(aiMsg);
    if(!c.modeLocked){if(!c.mode)c.mode=(S.uiMode==='workflow')?'workflow':'free';c.modeLocked=true;c.title=buildTitleWithSuffix(c.title,c.mode);}
    const bt=(c.title||'').replace(/-自由$/,'').replace(/-工作流$/,'').trim();if((bt===''||bt==='新对话')&&c.messages.length<=2)c.title=buildTitleWithSuffix(prompt.slice(0,24),chatMode(c));
    c.updatedAt=Date.now();renderMs();renderSB();await saveNow();
    const sendBtn=document.getElementById('sendBtn');sendBtn.classList.add('stop');sendBtn.textContent='■';
    const area=document.getElementById('msgsArea');const lastMsgEl=area.querySelector('.msg:last-child .bub');
    _streamCtrl=API.generateImage(profile,prompt,{size:'1024x1024',n:1},{
        onStart:()=>{},
        onImage:async(imgs)=>{const md=imgs.map((u,i)=>'![生成图片'+(i+1)+']('+u+')').join('\n\n');aiMsg.content=md;aiMsg._streaming=false;c.updatedAt=Date.now();_streamCtrl=null;sendBtn.classList.remove('stop');sendBtn.textContent='➤';if(lastMsgEl)UI.fullRender(lastMsgEl,md);await saveNow();renderMs();renderSB();if(typeof Archive!=='undefined')Archive.notifyActivity();},
        onError:async(err)=>{aiMsg.content='❌ 生图失败：'+err.message;aiMsg._streaming=false;aiMsg._interrupted=true;_streamCtrl=null;sendBtn.classList.remove('stop');sendBtn.textContent='➤';if(lastMsgEl)UI.fullRender(lastMsgEl,aiMsg.content);await saveNow();toast('生图失败：'+err.message,'er');},
    });
}

async function regenerate(msg){
    const c=curChat();if(!c)return;const idx=c.messages.indexOf(msg);if(idx<1)return;
    const prev=c.messages[idx-1];if(prev.role!=='user'){toast('无法找到对应的提问','er');return;}
    const actual=prev._actual||(typeof prev.content==='string'?prev.content:'');const visible=typeof prev.content==='string'?prev.content:'';
    c.messages.splice(idx,1);c.messages.splice(idx-1,1);await saveNow();renderMs();
...
async function _importShareFile(file){
    ...
    for(let attempt=0;attempt<3;attempt++){
        try{const result=await Snapshot.importSharedChat(file,password);const chat=result.chat;if(!chat.mode)chat.mode='free';chat.modeLocked=true;chat.folderId=chat.folderId||null;if(!Array.isArray(chat.refPool))chat.refPool=[];chat.title=buildTitleWithSuffix(chat.title,chat.mode);S.chats[chat.id]=chat;S.chatOrder.unshift(chat.id);S.currentChatId=chat.id;await saveNow();renderAll();toast('✅ 已导入分享对话');return;}
        catch(e){if(e.code==='NEED_PASSWORD'||/口令/.test(e.message)){const pwd=prompt(attempt===0?'该分享已加密，请输入口令：':'口令错误，重试（剩 '+(3-attempt)+' 次）：','');if(pwd===null){toast('已取消');return;}password=pwd.trim();continue;}toast('❌ 导入失败：'+e.message,'er');return;}
    }
    toast('❌ 口令错误次数过多','er');
}
...
async function handleUploadedFiles(files){
    ...
    for(const jf of jsonFiles){try{const ft=await Snapshot.detectFileType(jf);if(ft==='share'||ft==='enc'||ft==='enc-pwd'){await _importShareFile(jf);continue;}if(ft==='snapshot'){toast('📦 检测到快照文件，请通过 📦 快照迁移 导入','er');continue;}otherFiles.push(jf);}catch(e){otherFiles.push(jf);}}
    ...
    const contChk=document.getElementById('attCont');const wfContChk=document.getElementById('wfAttCont');
    ...
    if(isContinuous&&okCount>0){
        let c=curChat();if(!c){newChat();c=curChat();}
        if(c){if(!c.knowledgeBase)c.knowledgeBase=[];let addedToKB=0;results.forEach(r=>{if(r.ok){c.knowledgeBase.push({id:gId(),name:r.result.fileName,type:r.result.type,text:r.result.text||'',dataUrl:r.result.dataUrl||null,meta:r.result.meta||{},addedAt:Date.now()});const pi=_pendingAtts.indexOf(r.result);if(pi>-1)_pendingAtts.splice(pi,1);addedToKB++;}});c.updatedAt=Date.now();await saveNow();renderKBList();if(addedToKB>0)toast('📚 已加入知识库（'+addedToKB+' 个文件）');}
    }else{
        let added=0;results.forEach(r=>{if(r.ok){const pi=_pendingAtts.indexOf(r.result);if(pi>-1)_pendingAtts.splice(pi,1);addFileToRefPool(r.result);added++;}});if(added>0)toast('✅ 已解析 '+added+' 个文件，加入参考框（勾选即带）'+(failCount?'（'+failCount+' 失败）':''));
    }
    renderAttList();renderRefPool();if(typeof renderWfAtts==='function')renderWfAtts();
}
...
function renderAttList(){
    ...
    _pendingAtts.forEach((a,idx)=>{const item=document.createElement('div');item.className='att-item';const icon=a.type==='image'?'🖼️':a.type==='table'?'📊':a.type==='document'?'📄':'📝';const nm=document.createElement('span');nm.className='ai-nm';nm.textContent=icon+' '+a.fileName;item.appendChild(nm);const sz=document.createElement('span');sz.className='ai-sz';sz.textContent=a.type==='image'?(a.meta.sizeText||''):(cntW(a.text)+' 字');item.appendChild(sz);if(_attChunk&&a.type!=='image'&&a.text){const pv=document.createElement('button');pv.className='ai-rm';pv.textContent='👁';pv.title='预览打标';pv.style.color='var(--pri)';pv.onclick=()=>previewChunkObj(a);item.appendChild(pv);}const rm=document.createElement('button');rm.className='ai-rm';rm.textContent='×';rm.title='移除';rm.onclick=()=>{_pendingAtts.splice(idx,1);renderAttList();if(typeof renderWfAtts==='function')renderWfAtts();};item.appendChild(rm);list.appendChild(item);});
}
function clrAtt(){_pendingAtts=[];renderAttList();}

async function addKB(inputEl){
    if(!inputEl.files||!inputEl.files.length)return;const c=curChat();if(!c){toast('请先创建会话','er');return;}
    toast('解析知识库文件...');const results=await Parser.parseFiles(inputEl.files);
    ...
    if(ok>0){c.updatedAt=Date.now();await saveNow();toast('✅ 已加入 '+ok+' 个知识库文件');}renderKBList();inputEl.value='';
}
function renderKBList(){
    ...
    wrap.innerHTML='';
    c.knowledgeBase.forEach((k,idx)=>{const item=document.createElement('div');item.className='att-item';item.style.marginBottom='4px';const icon=k.type==='image'?'🖼️':k.type==='table'?'📊':k.type==='document'?'📄':'📝';const nm=document.createElement('span');nm.className='ai-nm';nm.textContent=icon+' '+k.name;item.appendChild(nm);const sz=document.createElement('span');sz.className='ai-sz';sz.textContent=k.type==='image'?'图片':(cntW(k.text||'')+' 字');item.appendChild(sz);if(_attChunk&&k.type!=='image'&&k.text){const pv=document.createElement('button');pv.className='ai-rm';pv.textContent='👁';pv.title='预览打标';pv.style.color='var(--pri)';pv.onclick=()=>previewChunkObj({fileName:k.name,type:k.type,text:k.text});item.appendChild(pv);}const rm=document.createElement('button');rm.className='ai-rm';rm.textContent='×';rm.title='移除';rm.onclick=async()=>{if(!confirm('从知识库移除 "'+k.name+'"？'))return;c.knowledgeBase.splice(idx,1);c.updatedAt=Date.now();await saveNow();renderKBList();};item.appendChild(rm);wrap.appendChild(item);});
}

/* ===== 快照 / 导入导出 ===== */
function eSnap(){const includeKey=confirm('导出快照\n\n✅=含私有引擎Key\n❌=不含');Snapshot.exportToFile(S,{includeKey:includeKey});}
async function iSnap(inputEl){
    if(!inputEl.files||!inputEl.files.length)return;const file=inputEl.files[0];
    try{await DB.saveRollbackBackup(S);}catch(e){}
    const mode=confirm('✅=替换模式\n❌=合并模式');
    try{
        const {state:imp,source}=await Snapshot.importFromFile(file);
        let finalState;
        if(mode){const {state:ps}=Snapshot.protectLocalKeys(imp,S);finalState=ps;}
        else{const {state:ps}=Snapshot.protectLocalKeys(imp,S);finalState=Snapshot.mergeStates(S,ps);}
        S=finalState;
        if(!Array.isArray(S.folders))S.folders=[];if(!S.folderCollapsed)S.folderCollapsed={};if(!S.defaultMode)S.defaultMode='free';
        for(const cid in S.chats){if(!Array.isArray(S.chats[cid].refPool))S.chats[cid].refPool=[];}
        S._engineMigrated=true;  // 导入的快照不再触发清空
        fixProfileFields();
        await saveNow();await Snapshot.snapNow(S);renderAll();
        toast('✅ 导入成功（'+source+'）：'+Object.keys(S.chats||{}).length+' 个会话');closeM('snap');
    }catch(e){toast('导入失败：'+e.message,'er');}
    inputEl.value='';
}
async function rollbackImport(){
    let bak=null;try{bak=await DB.loadRollbackBackup();}catch(e){}
    if(!bak||!bak.state){toast('暂无可恢复的备份','er');return;}
    const t=bak.ts?new Date(bak.ts).toLocaleString():'未知';
    if(!confirm('⏪ 恢复到上次导入前？\n备份时间：'+t))return;
    try{S=bak.state;if(!Array.isArray(S.folders))S.folders=[];if(!S.folderCollapsed)S.folderCollapsed={};if(!S.defaultMode)S.defaultMode='free';for(const cid in S.chats){if(!Array.isArray(S.chats[cid].refPool))S.chats[cid].refPool=[];}S._engineMigrated=true;fixProfileFields();if(!S.currentChatId||!S.chats[S.currentChatId]){S.currentChatId=(S.chatOrder&&S.chatOrder[0])||null;}await saveNow();await DB.clearRollbackBackup();renderAll();toast('✅ 已恢复');closeM('snap');}catch(e){toast('恢复失败：'+e.message,'er');}
}
...
async function initApp(){
    ...
    try{
        ...
        await loadPublicEngines();   // ★ 拉公有引擎
        await loadGlobalConfig();    // ★ 拉全局配置（打标块大小）
...
async function loadGlobalConfig(){
    try{
        ...
        const data=await resp.json();
        if(data.ok&&data.config&&data.config.chunkSize&&typeof Chunker!=='undefined'){Chunker.setBlockSize(data.config.chunkSize);}
    }catch(e){}
}
function initUpload(){
