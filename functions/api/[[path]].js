...
async function hVerify(request,env){
    ...
    if(env.DB){
        try{
            const u=await env.DB.prepare('SELECT name,role,status,permissions FROM users WHERE username=?').bind(pl.username).first();
            if(u){perm=u.permissions||'{}';role=u.role;name=u.name;status=u.status;}
            const ip=request.headers.get('CF-Connecting-IP')||'';
...
/* ---------- 用户：公有引擎（不含Key，含用户选中的model） ---------- */
async function hEngines(request,env){
    const pl=await verifyUser(request,env);if(!pl)return jr({error:'未登录'},401);
    try{
        const rows=(await env.DB.prepare('SELECT id,name,protocol,model,user_model,price_in,price_out,price_cache_read,price_cache_write FROM engines_public WHERE username=? ORDER BY name').bind(pl.username).all()).results||[];
        const engines=rows.map(e=>({id:e.id,name:e.name,protocol:e.protocol,model:e.user_model||e.model||'',priceIn:e.price_in,priceOut:e.price_out,priceCacheRead:e.price_cache_read,priceCacheWrite:e.price_cache_write,origin:'public'}));
        return jr({ok:true,engines});
    }catch(e){
        // 兼容：user_model列可能不存在（旧表），降级查询
        try{
...
async function hEngineModels(request,env,url){
    ...
    try{
        ...
        let path='models';if(e.protocol==='anthropic')path='v1/models';if(e.protocol==='gemini')path='v1beta/models';
...
async function hSetModel(request,env){
    ...
    try{
        await env.DB.prepare('UPDATE engines_public SET user_model=? WHERE id=? AND username=?').bind(b.model||'',b.engineId,pl.username).run();
        return jr({ok:true});
    }catch(e){return jr({ok:false,error:e.message});}  // user_model列不存在时静默失败
}
/* 用户：获取预设 */
async function hGetPresets(request,env){
    const pl=await verifyUser(request,env);if(!pl)return jr({error:'未登录'},401);
    try{const row=await env.DB.prepare('SELECT data FROM presets WHERE id=1').first();if(!row||!row.data)return jr({ok:true,presets:null});return jr({ok:true,presets:JSON.parse(row.data)});}catch(e){return jr({ok:true,presets:null});}
}
/* 用户：监视上报 */
async function hLog(request,env){
    const pl=await verifyUser(request,env);if(!pl)return jr({error:'未登录'},401);
    let b;try{b=await request.json();}catch(e){return jr({error:'格式错误'},400);}
    try{await env.DB.prepare('INSERT INTO logs (username,chat_name,rounds,tokens,model,created_at) VALUES (?,?,?,?,?,?)').bind(pl.username,(b.chatName||'').slice(0,100),b.rounds||0,b.tokens||0,(b.model||'').slice(0,60),Date.now()).run();return jr({ok:true});}catch(e){return jr({ok:false});}
}
/* 用户：全局配置 */
...
async function aUsersCreate(request,env){
    ...
    try{if(await env.DB.prepare('SELECT id FROM users WHERE username=?').bind(un).first())return jr({error:'账号已存在'},400);const h=await sha256(pw);await env.DB.prepare('INSERT INTO users (username,password_hash,name,role,status,permissions,created_at) VALUES (?,?,?,?,?,?,?)').bind(un,h,nm,role,'active','{}',Date.now()).run();return jr({ok:true});}catch(e){return jr({error:e.message},500);}
}
async function aUsersUpdate(request,env){
    let b;try{b=await request.json();}catch(e){return jr({error:'格式错误'},400);}
    const un=(b.username||'').trim();if(!un)return jr({error:'缺账号'},400);
    try{const f=[],v=[];if(b.name!==undefined){f.push('name=?');v.push(b.name);}if(b.role!==undefined){f.push('role=?');v.push(b.role==='admin'?'admin':'user');}if(b.status!==undefined){f.push('status=?');v.push(b.status==='active'?'active':'disabled');}if(!f.length)return jr({error:'无更新'},400);v.push(un);await env.DB.prepare('UPDATE users SET '+f.join(',')+' WHERE username=?').bind(...v).run();return jr({ok:true});}catch(e){return jr({error:e.message},500);}
}
async function aUsersDelete(request,env){
    let b;try{b=await request.json();}catch(e){return jr({error:'格式错误'},400);}
    const un=(b.username||'').trim();if(!un)return jr({error:'缺账号'},400);if(un==='admin')return jr({error:'不能删admin'},400);
    try{await env.DB.prepare('DELETE FROM users WHERE username=?').bind(un).run();await env.DB.prepare('DELETE FROM engines_public WHERE username=?').bind(un).run();await env.DB.prepare('DELETE FROM sessions WHERE username=?').bind(un).run();return jr({ok:true});}catch(e){return jr({error:e.message},500);}
}
async function aUsersResetPwd(request,env){
    let b;try{b=await request.json();}catch(e){return jr({error:'格式错误'},400);}
    const un=(b.username||'').trim(),pw=(b.password||'').trim();if(!un||!pw)return jr({error:'必填'},400);
...
async function aUsersPerm(request,env){
    let b;try{b=await request.json();}catch(e){return jr({error:'格式错误'},400);}
    const un=(b.username||'').trim();if(!un)return jr({error:'缺账号'},400);
    try{await env.DB.prepare('UPDATE users SET permissions=? WHERE username=?').bind(JSON.stringify(b.permissions||{}),un).run();return jr({ok:true});}catch(e){return jr({error:e.message},500);}
}
/* xlsx导入（前端已解析成rows对象数组） */
async function aUsersImport(request,env){
    ...
    rows.forEach(r=>{
        const un=String(r['账号']||'').trim();if(!un)return;
        if(!um[un])um[un]={username:un,password:String(r['密码']||'').trim(),name:String(r['姓名']||'').trim(),role:String(r['角色']||'user').trim()==='admin'?'admin':'user',engines:[]};
        const en=String(r['引擎名称']||'').trim();
        if(en)um[un].engines.push({name:en,protocol:String(r['协议']||'openai').trim(),base:String(r['BaseURL']||'').trim(),key:String(r['APIKey']||'').trim(),model:String(r['模型']||'').trim(),pi:parseFloat(r['输入单价'])||0,po:parseFloat(r['输出单价'])||0,pcr:parseFloat(r['缓存读单价'])||0,pcw:parseFloat(r['缓存写单价'])||0});
    });
    for(const un in um){const u=um[un];try{
        if(!u.password){errs.push(un+'：缺密码');continue;}
        const h=await sha256(u.password);
        if(await env.DB.prepare('SELECT id FROM users WHERE username=?').bind(un).first())await env.DB.prepare('UPDATE users SET password_hash=?,name=?,role=? WHERE username=?').bind(h,u.name,u.role,un).run();
        else await env.DB.prepare('INSERT INTO users (username,password_hash,name,role,status,permissions,created_at) VALUES (?,?,?,?,?,?,?)').bind(un,h,u.name,u.role,'active','{}',Date.now()).run();
        uc++;
        await env.DB.prepare('DELETE FROM engines_public WHERE username=?').bind(un).run();
        for(const eng of u.engines){const eid='eng_'+un+'_'+Math.random().toString(36).slice(2,8);const ke=await encKey(eng.key,env.KEY_SECRET);await env.DB.prepare('INSERT INTO engines_public (id,username,name,protocol,base_url,api_key,model,price_in,price_out,price_cache_read,price_cache_write,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)').bind(eid,un,eng.name,eng.protocol,eng.base,ke,eng.model,eng.pi,eng.po,eng.pcr,eng.pcw,Date.now()).run();ec++;}
...
async function aEnginesList(request,env,url){
    ...
    try{
        let rows;
        if(un)rows=(await env.DB.prepare('SELECT id,username,name,protocol,base_url,model,price_in,price_out,price_cache_read,price_cache_write FROM engines_public WHERE username=? ORDER BY name').bind(un).all()).results||[];
        else rows=(await env.DB.prepare('SELECT id,username,name,protocol,base_url,model,price_in,price_out,price_cache_read,price_cache_write FROM engines_public ORDER BY username,name').all()).results||[];
...
async function aEnginesSave(request,env){
    ...
    try{
        ...
        if(ex)await env.DB.prepare('UPDATE engines_public SET name=?,protocol=?,base_url=?,api_key=?,model=?,price_in=?,price_out=?,price_cache_read=?,price_cache_write=?,updated_at=? WHERE id=?').bind(b.name,b.protocol||'openai',b.base||'',keyStored,b.model||'',b.priceIn||0,b.priceOut||0,b.priceCR||0,b.priceCW||0,Date.now(),b.id).run();
        else await env.DB.prepare('INSERT INTO engines_public (id,username,name,protocol,base_url,api_key,model,price_in,price_out,price_cache_read,price_cache_write,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)').bind(id,un,b.name,b.protocol||'openai',b.base||'',keyStored,b.model||'',b.priceIn||0,b.priceOut||0,b.priceCR||0,b.priceCW||0,Date.now()).run();
...
async function aPresetsGet(env){
    try{const row=await env.DB.prepare('SELECT data FROM presets WHERE id=1').first();return jr({ok:true,presets:row&&row.data?JSON.parse(row.data):null});}catch(e){return jr({ok:true,presets:null});}
}
...
async function aMonitor(env){
    try{
        const dayAgo=Date.now()-5*60*1000;
        const online=(await env.DB.prepare('SELECT COUNT(DISTINCT username) AS n FROM sessions WHERE last_active>?').bind(dayAgo).first())||{n:0};
        const weekAgo=Date.now()-7*24*3600*1000;
        ...
        return jr({ok:true,onlineCount:online.n||0,sessions:sess,logs:logs,recent:recent});
    }catch(e){return jr({error:e.message},500);}
}
...
async function hProxy(request,env,url,sub){
    ...
    if(engineId){
        ...
        targetBase=e.base_url;apiKey=await decKey(e.api_key,env.KEY_SECRET);
